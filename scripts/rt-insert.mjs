...
const {error}=await a.from("messages").insert({conversation_id:conv.id,direction:"inbound",type:"text",body:"REALTIME TEST - did this appear without refresh?",status:"received",created_at:now});
await a.from("conversations").update({last_message_at:now,last_inbound_at:now}).eq("id",conv.id);
console.log("inserted:",error?error.message:"OK into conversation "+conv.id);
