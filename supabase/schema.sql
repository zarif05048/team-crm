...
create table if not exists public.conversations (
  ...
  -- Denormalised from the newest message so the inbox list is one small query
  -- instead of a 1,500-row scan of `messages` on every render. Maintained by
  -- messages_touch_conversation below — never write these from the app.
  last_message_body      text,                     -- preview line, first 160 chars
...
-- Migration for databases created before 2026-07-26 (list preview/unread):
alter table public.conversations
...
-- ----------------------------------------------------------------------------
-- This trigger is the ONLY writer of last_message_at / last_inbound_at / the
-- preview columns. Insert the message and the conversation follows; an app-side
-- update would just fire a second realtime event for the same message.
-- Full rationale: supabase/migrations/2026-07-26_conversation_preview.sql
...
-- ----------------------------------------------------------------------------
-- Weight-loss pipeline: tag weight-loss enquiries and start them in "new"
-- (migration 2026-09-23_weight_loss_pipeline.sql, which also backfills)
-- ----------------------------------------------------------------------------
